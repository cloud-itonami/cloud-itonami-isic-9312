(ns sportsclub.operation
  "OperationActor -- one sports-club operation = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (ClubOps-LLM) is sealed into a single node (:advise); its proposal
  is ALWAYS routed through the Membership Governor (:govern) and the
  ...
  One graph run = one sports-club operation (intake -> advise -> govern
  ...
  decision to a human operator (club officer). The approver resumes
  with `{:approval {:status :approved}}` (or :rejected).
  `:actuation/finalize-membership-action` ALWAYS reaches this node when
  the governor is clean -- see `sportsclub.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [sportsclub.clubopsllm :as clubopsllm]
...
(defn build
  ...
  `sportsclub.store/Store`).
  opts:
    :advisor      -- a `sportsclub.clubopsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (clubopsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; ClubOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (clubopsllm/-advise advisor store request)]
            {:proposal p :audit [(clubopsllm/trace request p)]})))

      ;; Membership Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
